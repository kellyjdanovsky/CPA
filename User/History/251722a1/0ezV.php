...
<table style="width:100%; border-collapse:collapse; border: 1px solid #000; margin: 10px auto;" border="1">
    ...
    <td class="moyen_sans_coef">
        ...
        {{ number_format($moyen_sans_coef, 2) }} <!-- Afficher la moyenne sans coefficient -->
    </td>

    <!-- Calcul de la moyenne pondérée avec coefficient -->
    <td class="notetotalaveccoef">
        @php
            // Récupérer le coefficient de la matière
            $coef = $sub->coef ?: 0; // Si le coefficient est null, on le considère comme 0
            
            // Calcul de la moyenne pondérée
            // On multiplie chaque note par le coefficient de la matière
            $weighted_sum = ($t1 * $coef) + ($t2 * $coef) + ($exm * $coef);
            $coef_count = $coef * 3; // Chaque note pèse le même coefficient, donc 3 fois le coefficient total
            
            // Calcul de la moyenne pondérée (si la somme des coefficients est supérieure à 0)
            $moyen_avec_coef = $coef_count > 0 ? $weighted_sum / $coef_count : 0;
        @endphp

        {{ number_format($moyen_avec_coef, 2) }} <!-- Afficher la moyenne pondérée avec coefficient -->
    </td>
