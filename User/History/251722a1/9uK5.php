    ...
    <tr>
        ...
        @foreach ($marks->where('subject_id', $sub->id)->where('exam_id', $ex->id) as $mk)
            ...
            <td class="notetotalaveccoef">
                @php
                    // Si le coefficient est supérieur à 0, on calcule la moyenne pondérée
                    $coef = $sub->coef ?: 0; // S'assurer que le coefficient est valide (par défaut à 0 si non défini)
                    
                    // Calcul de la moyenne pondérée (on fait un somme des notes multipliées par le coefficient)
                    $weighted_sum = ($t1 * $coef) + ($t2 * $coef) + ($exm * $coef);
                    $coef_count = $coef * 3; // Chaque note pèse en fonction du coefficient
