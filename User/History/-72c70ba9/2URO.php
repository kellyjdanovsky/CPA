                        ...
                        <div class="row">
                            <div class="col-md-10">
                                <div class="form-group">
                                    ...
                                    <select required id="my_class_id" name="my_class_id" class="form-control select">
                                        ...
                                        @foreach($class as $c)
                                            <option name="class" value="{{ $c->id }}">{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select required id="payments" name="my_class_id" class="form-control select">
                                        <option value="">Choisir une payments</option>
                                            ...
                                            <option name="payments" id="$payments" value="{{ $c->id }}">{{ $c->title }}</option>
                            ...
                            <div class="col-md-2 mt-4">
                                <div class="text-right mt-1">
                                    <button  id="letgo" type="submit" class="btn btn-primary">Valider <i class="icon-paperplane ml-2"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function(){
        $('#letgo').click(function(){
            var classId = $('select[name="class"]').val(); // Get the value of class ID by name
            var paymentId = $('select[name="payments"]').val(); // Get the value of payment ID by name

            if(classId && paymentId){
                $.ajax({
                    type: "GET",
                    url: "{{ route('payments.select') }}",
                    data: {
                        class_id: classId,
                        payment_id: paymentId // Include payment ID in the data
                    }
                });
            } else {
                alert("Class or payment not selected");
            }
        });
    });
</script>
