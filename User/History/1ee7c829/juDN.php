...
@section('content')
                ...
                <div class="card card-body bg-blue-400 has-bg-image">
                    <div class="media">
                        <div class="media-body">
                            <h3 class="mb-0 solde">{{ $solde }} Ar</h3>
                    ...
                    @foreach ($encaissement as $payment)
                        @if ($payment->amt_paid)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $s_r = \App\Models\StudentRecord::where('user_id',$payment->student->id)->first();
                                      
                                        $class = $s_r->my_class->name;
                ...
                <tbody>
                    @foreach ($decaissement as $payment)
                        <tr>
                            ...
                            <td>
                                @if ($payment->piece)
                                    <img style="width: 50px" src="{{ asset('images/' . $payment->piece) }}"
                                        alt="Piece justificatif">
                                @else
                                    Pas de piece
                                @endif
                            </td>
                            ...
                            <td><a href="" data-id="{{ $payment->ref }}" data-motif="{{ $payment->motif }}"
                                    data-montant="{{ $payment->montant }}" data-annee="{{ $payment->annee }}"
                                    data-created_at="{{ $data_created_at }}" class="btn btn-primary recu-button"
                                    target="_blank">Recu</a>
                            <td>
    ...
    <!-- Modal -->
    <!-- Modal -->
    <div class="modal fade" id="decaissementModal" tabindex="-1" role="dialog" aria-labelledby="decaissementModalLabel"
        aria-hidden="true">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{ route('payments.decaissement') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control dcm" min="10" max="" id="montant"
    ...
    <script>
        $(document).ready(function() {
            ...
            var soldeValue = parseInt($(".solde").text());
            ...
            recuButtons.forEach(function(button) {
                button.addEventListener("click", function(e) {
                    ...
                    printWindow.document.write(
                        `<html><head><title>Reçu_decaissement_${id}</title></head><body>`);
                    ...
                    printWindow.document.write(
                        `<div><strong>Montant:</strong> ${montant} Ariary</div>`);
                    printWindow.document.write(
                        `<div><strong>Date:</strong> ${created_at} session ${annee}</div>`);
