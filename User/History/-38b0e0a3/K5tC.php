...
class PaymentController extends Controller
{
    ...
    {
        //section montant
        $prM = PaymentRecord::sum('amt_paid');
        $decaissement_montant = decaissement::sum('montant');
        $solde = $prM - $decaissement_montant;

        //section liste encaissement et decaisemment
        $encaissement = PaymentRecord::orderBy('created_at', 'desc')->get();
        $decaissement = Decaissement::orderBy('created_at', 'desc')->get();

        return view('pages.support_team.payments.caisse', ['encaissement' => $encaissement, 'decaissement' => $decaissement, 'decM' => $decaissement_montant, 'solde' => $solde]);
    }
    public function decaissement(Request $request)
    {
        ...
        $request->validate([
            'montant' => 'required',
            'motif' => 'required',
            'piece' => 'image|mimes:jpeg,png,jpg,gif|max:4096', // Adjust the validation rules as needed
        ]);
        //dd($request->all());
        // Get the count of existing decaissement records
        $count = decaissement::count();

        // Increment the count to get the next unique code
        $code = $count + 1;
        $code = "ref-N°".$code."decCpa";

        $decaissement = new decaissement;

        $decaissement->montant = $request->montant;
        $decaissement->motif = $request->motif;
        $decaissement->annee = $this->year;
        $decaissement->ref = $code;// Pass the $decaissement data to the view for rendering
         // Handle image upload
    if ($request->piece) {
        $image = $request->file('piece');
        $imageName = time() . '.' . $image->getClientOriginalExtension();
        $image->move(public_path('images'), $imageName);
        $decaissement->piece = $imageName;
    }
       ...
       // return view('pages.support_team.payments.decrecu', ['dec' => $decaissement]);
      return redirect()->route('payments.caisse');

    }
    public function decaissementPrint()
{
    $id = request('id');
    //dd($id);
...
public function select(Request $request)
{
    ...
    $payments = Payment::where('my_class_id', $classId)->get();
    // Assuming your Payment model has 'id' and 'title' attributes
...
public function filter(Request $request)
{
    ...
    $studentRecord = StudentRecord::where('my_class_id', $class)->where('id',$p->student_id)->first();
    $pr = PaymentRecord::where('payment_id', $request->my_paymets_id)->get();
    return view('pages.support_team.payments.filter',['pr' => $pr, 'class'=>$request->my_class_id]);
}
